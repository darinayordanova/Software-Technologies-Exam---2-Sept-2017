...
public class ProductController {
	...
	public String createProcess(Model model, ProductBindingModel productBindingModel) {
		...
		if(productBindingModel.getPriority() == null || productBindingModel.getName().equals("")
				|| productBindingModel.getQuantity() == null ){
			model.addAttribute("product", productBindingModel);
			model.addAttribute("view", "product/create"); // Redirect-ваме към product/create
			return "base-layout";
		}

		Product film = new Product(productBindingModel.getPriority(), productBindingModel.getName(), productBindingModel.getQuantity(), productBindingModel.getStatus());

		productRepository.saveAndFlush(film); // Запазваме в Базата
		return "redirect:/";
	}

	@GetMapping("/edit/{id}")
	public String edit(Model model, @PathVariable int id) {
		Product product = productRepository.findOne(id); // Намираме конкретния Product по ID
	...
	public String editProcess(Model model, @PathVariable int id, ProductBindingModel productBindingModel) {
		...
		{
			model.addAttribute("product", productBindingModel);
			model.addAttribute("view", "product/edit"); // Redirect-ваме към product/edit
